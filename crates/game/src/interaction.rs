...
pub fn interaction_detection_system(
    ...
    for (interactor_net_id, interactor_entity, interactor_transform, interaction_input) in order_iter!(interactors) {
            ...
            order_iter!(interactables)
        {
            ...
            // Convert range to FixedWide for comparison (via f32 to normalize formats)
            let range_f32 = fixed_math::to_f32(interactable.interaction_range);
            let range_fw = fixed_math::FixedWide::from_num(range_f32);
...
pub fn display_interaction_prompts(
    ...
    for interactor_transform in interactors.iter() {
        for (_interactable_entity, interactable_transform, interactable) in interactables.iter() {
            ...
            // We need to convert through f32 to ensure proper scaling between Fixed (16.16) and FixedWide (32.32)
            let range_f32 = fixed_math::to_f32(interactable.interaction_range);
            let range_fw = fixed_math::FixedWide::from_num(range_f32);
            let range_sq_fw = range_fw.saturating_mul(range_fw);
